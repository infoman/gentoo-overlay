...
src_install() {
	...
	use nginx_modules_http_passenger	&& doins "${FILESDIR}"/03_passenger.conf
	...
	if use nginx_modules_http_passenger; then
		...
		for RUBY in $(ruby_get_use_implementations); do
			...
			insopts -m 0755
			doins buildout/ruby/*/passenger_native_support.so
			doruby -r lib/phusion_passenger lib/phusion_passenger.rb
		done

		exeinto /usr/bin
		doexe bin/{passenger-memory-stats,passenger-status,passenger-config}

		doman man/{passenger-config.1,passenger-memory-stats.8}
		doman man/{passenger-status.8,passenger-stress-test.1}

		exeinto /usr/libexec/passenger/bin
		doexe helper-scripts/prespawn

		exeinto /usr/libexec/passenger/buildout/agents
		doexe buildout/agents/Passenger{LoggingAgent,HelperAgent,Watchdog}
		doexe buildout/agents/SpawnPreparer

		insinto /usr/libexec/passenger
		doins -r helper-scripts resources
	fi
}
